...
/**
 * Returns the feature properties corresponding to the given locales.
 */
function getNameFields(locales, options = {}) {
  return [
    ...locales.flatMap((l) => {
      ...
      let fields = [[l, localizedNameProperty]];
      ...
      if (options.includesLegacyFields && (l === "de" || l === "en")) {
        fields.push([l, localizedNameProperty.replaceAll(":", "_")]);
      }
      return fields;
    }),
    ["mul", options.unlocalizedNameProperty || defaultUnlocalizedNameProperty],
  ];
}

/**
 * Returns a `coalesce` expression that evaluates to the feature's name in a
...
export function getLocalizedNameExpression(locales, options = {}) {
  const nameFields = getNameFields(locales, options);
  return ["coalesce", ...nameFields.map(([l, f]) => ["get", f])];
}

/**
 * Returns a `case` expression that evaluates to the language of the expression
 * generated by `getLocalizedNameExpression()`.
 *
 * @param {string[]} locales - Locales of the name fields to include.
 * @param {object} options
 * @param {boolean} [options.includesLegacyFields] - Whether to include the older fields
 ...
 * @returns {Expression}
 */
export function getResolvedLocaleExpression(locales, options = {}) {
  const nameFields = getNameFields(locales, options);
  return ["case", ...nameFields.flatMap(([l, f]) => [["has", f], l]), null];
}
